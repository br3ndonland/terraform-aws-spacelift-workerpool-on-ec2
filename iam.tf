...
resource "aws_iam_role_policy_attachment" "secure_strings" {
  count      = local.secure_strings_count
  role       = aws_iam_role.this[0].name
  policy_arn = aws_iam_policy.secure_strings[count.index].arn
}
...
data "aws_iam_policy_document" "secure_strings" {
  count = local.secure_strings_count
  statement {
    effect = "Allow"
    actions = [
  ...
  dynamic "statement" {
    for_each = var.secure_strings_kms_key_id != null ? ["USE_KMS_KEY"] : []
    content {
      effect = "Allow"
      actions = [
        "kms:Decrypt",
      ...
      resources = [var.secure_strings_kms_key_id]
    }
  }
}

resource "aws_iam_policy" "secure_strings" {
  count       = local.secure_strings_count
  name        = "${local.base_name}-secure-strings"
  description = "Allows access to the secure strings stored in Secrets Manager"
  policy      = data.aws_iam_policy_document.secure_strings[count.index].json
}
